...
p <-
  p + ggtitle(
    paste0("Glasses.com OOS % by Contribution Code - Updated", " ", Sys.Date()))
...
p <- p + facet_wrap( ~ facet, nr = 2) +
  theme(
    strip.background = element_rect(fill = "#005192"),
...
p <-
  p + ggtitle(paste0("Historical ATP OOS % by CC - Updated", " ", Sys.Date()))
p <- p + theme(plot.title = element_text(size = 12))
